...
class Go1Env():
    ...
    def run_robot(self) -> None:
        ...
        # Option 3:
        """
        q_robot = self.policy_to_robot_joint_reorder(self.q) + self.q_stand
        dq_robot = self.policy_to_robot_joint_reorder(self.dq)
        for motor_id in range(12):
            q_act = self.stance_trigger * self.ka * a_cmd_robot[motor_id]
            q_err = q_act - q_robot[motor_id]
            dq_err = - dq_robot[motor_id]
            # self.lowcmd.motorCmd[motor_id].Kp = 0
            # self.lowcmd.motorCmd[motor_id].Kd = 0
            # self.lowcmd.motorCmd[motor_id].tau = self.kp * q_err + self.kd * dq_err
            print(self.kp * q_err + self.kd * dq_err)
        """
...
def main() -> None:
    ...
    for _ in range(5 * int(1 / POLICY_STEP)):
        obs, lowstate = env.step(a_cmd, gait_mode, np.array([0, 0, 0], dtype=np.float32))
